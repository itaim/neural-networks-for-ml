...
function test_gradient(model, data, wd_coefficient)
  ...
  model_loss = d_loss_by_d_model(model, data, wd_coefficient);
  analytic_gradient = model_to_theta(model_loss);
  fprintf(' sizes \n');
  fprintf(' input_to_hid: %d, %d hid_to_class: %d, %d \n',size(model_loss.input_to_hid,1),size(model_loss.input_to_hid,2),size(model_loss.hid_to_class,1),size(model_loss.hid_to_class,2));
  fprintf(' analytic_gradient: %d \n',size(analytic_gradient,1))
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  hid_input = model.input_to_hid * data.inputs; % dimensions: hidden by cases  
  hid_output = logistic(hid_input) % dimensions: hidden by cases
  class_input = model.hid_to_class * hid_output; % dimensions: classes by cases
  class_normalizer = log_sum_exp_over_rows(class_input);
  log_class_prob = class_input - repmat(class_normalizer,[size(class_input,1),1]); % dimensions: classes by cases
  class_prob = exp(log_class_prob); % dimensions: classes by cases
  
  %we are using the L2 norm, which is (1/2)a*w^2, where a is the wd_coefficient and w are the weights between layers
  N = max(size(data.targets, 2),1);
  ret.hid_to_class = ((class_prob - data.targets) * hid_output' ./ N) + (wd_coefficient * model.hid_to_class); 
  %                         10 X 1000              1000 X 7
  ret.input_to_hid = (((model.hid_to_class' * (class_prob - data.targets)) .*...
  (hid_output.*(1-hid_output)) * data.inputs') ./ N) + (wd_coefficient * model.input_to_hid);
  %                         7 X 10                  10 X 1000                    7 X 1000                  1000 X 256    
end
